...
export function Navbar() {
    ...
    return (
        <>
                ...
                <div className="flex items-center gap-6">
                    ...
                    <Sheet open={isMenuOpen} onOpenChange={setIsMenuOpen}>
                        <SheetTrigger asChild>
                            <button className="md:hidden text-white hover:text-gold transition-colors p-2 -ml-2">
                        ...
                        <SheetContent side="left" className="bg-black border-r border-gold/10 w-full p-0 flex flex-col">
                            ...
                            <div className="flex-grow flex flex-col justify-center gap-8 py-12 text-sm uppercase tracking-[0.5em] font-light text-center px-8">
                                ...
                                <Link
                                    ...
                                    className="text-gold/40 hover:text-gold transition-colors py-2 flex items-center justify-center gap-3"
                                >
                                    <User size={14} />
                                    Admin Vault
                                </Link>
                            ...
                            <div className="p-8 border-t border-white/5 bg-zinc-950/50 space-y-8">
                                ...
                                <Link
                                    href="/shop"
                                    onClick={() => setIsMenuOpen(false)}
                                    className="w-full border border-gold/40 py-4 flex items-center justify-center gap-4 text-gold uppercase text-[10px] tracking-[0.3em]"
                                >
            ...
            {isSearchOpen && (
                <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-xl animate-in fade-in duration-500 overflow-y-auto">
                    <button
                        onClick={() => {
                            setIsSearchOpen(false);
                            setSearchQuery("");
                        }}
                        className="fixed top-6 right-6 md:top-10 md:right-10 text-white/40 hover:text-gold transition-colors z-[110]"
                    >
                        <X size={32} />
                    </button>
                    ...
                    <div className="max-w-3xl mx-auto mt-24 md:mt-40 px-6 space-y-12 pb-20">
                        <div className="space-y-4">
                            ...
                            <input
                                ...
                                className="w-full bg-transparent border-b border-gold/20 py-4 md:py-6 text-xl md:text-5xl font-serif text-white outline-none focus:border-gold transition-colors placeholder:text-zinc-800"
                            />
                        </div>
                        ...
                        <div className="grid grid-cols-1 gap-4">
                            {searchResults.map((product) => (
                                <Link
                                    ...
                                    className="flex items-center gap-4 md:gap-6 p-3 md:p-4 border border-white/5 hover:border-gold/20 bg-zinc-950/50 transition-all group"
                                >
                                    ...
                                    <div className="flex-grow">
                                        <h3 className="text-xs md:text-sm font-serif text-white group-hover:text-gold transition-colors line-clamp-1">{product.name}</h3>
